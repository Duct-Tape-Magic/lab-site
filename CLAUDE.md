...
Added 2026-09-23: account-locked access is live without an Edge Function. `item_groups` (classes/groups) and `item_users` (individual Google accounts) attach to the governing item (the lab or folder whose access is 'accounts'); `student_allowed(governing_id)` checks both. RLS now HIDES accounts-locked folders/labs/content from anyone not allowed (`item_visible`, `resolve_folder_governing`), so a student only ever receives their own classes' labs; the home page shows an editable log-in nudge (`homeLoginNudge`) to visitors. Admin UI: the "Who can open it" select on labs and folders reveals an access list (classes as checkboxes, Google accounts with a filter) saved by `saveAccessList` after the item upsert; choices come from `list_classes` and `list_account_choices`. Remaining gap: files of locked labs still live in the public bucket, so a copied file URL works for anyone; the private-bucket + signed-URL move is still to do. Password level is still unbuilt (needs the gate function).
